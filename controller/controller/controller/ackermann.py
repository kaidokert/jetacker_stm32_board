...
class AckermannChassis:
    # wheelbase = 0.213  #  前后轴距(Distance between the front and rear axles)
    # track_width = 0.222  # 左右轴距(Distance between the left and right axles)
    ...
    def set_velocity(self, linear_speed, angular_speed, reset_servo=True):
        ...
        if abs(linear_speed) >= 1e-8:
            if abs(angular_speed) >= 1e-8:
                ...
                if steering_angle > math.radians(37):
                    steering_angle = math.radians(37)
                elif steering_angle < math.radians(-37):
                    steering_angle = math.radians(-37)
                    # for i in range(4):
                        # msg = MotorState()
                        # msg.id = i + 1
